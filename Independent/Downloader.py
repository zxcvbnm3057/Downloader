...
class download_manager(object):
    ...
    def run(self):
        ...
        if "HTTP/1.1 200 OK" in self.data:
            if "Content-Length:" in self.data:
                length = eval(self.data.split("Content-Length:")[1].split("\r\n", 1)[0])
                block_size = 20971520
                block_number = length // block_size + 1 if length % block_size else 0
                makedirs(self.save_file.rsplit(".", 1)[0])
                for i in range(block_number):
                    self.task_list.append(self.executor.submit(self.download, eval(self.s_init), i, i * block_size, length % block_size if i == block_number - 1 else block_size ))
            else:
                self.task_list.append(self.executor.submit(self.download, eval(self.s_init)))
            wait(self.task_list)
    ...
    def download_continue(self, pause_data):
        for items in pause_data:
            self.task_list.append(self.executor.submit(self.download,eval(self.s_init),id=items["id"],begin=items["begin"]+items["recieved"],length=items["length"]))
